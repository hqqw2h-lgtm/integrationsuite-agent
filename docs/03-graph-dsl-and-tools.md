...
````markdown
## iFlow: REST IncomingInvoice to S4 XI
...
```mermaid
flowchart LR
  Start --> SetHeaders --> LogRequest --> GetUrlParam --> JsonToXml --> SetProps --> RequestReply --> XmlToJson --> SetReceiver --> LogResponse --> End
```

### Channels

- sender `REST_IncomingInvoice_Sender`: HTTPS, path=`{{MessageFlow_1_urlPath}}`
- receiver `SOAP_Receiver_MM`: XI over HTTP, QoS=`{{MessageFlow_3_QualityOfService}}`

### Open Issues

- `receiver.qualityOfService` is externalized and has no default.
````
...
```json
{
  ...
  "arguments": {
    "iflowId": "iflow_001",
    "targetRevision": 9,
    "reason": "Auto-fix introduced invalid receiver channel"
  }
}
```
...
`updateEdge` 必须采用 schema 化 replace/patch：

```json
{
  "iflowId": "iflow_001",
  "edgeRef": {
    "semanticKey": "process.main/edge/JsonToXml_to_SetProps"
  },
  "replaceWith": {
    "type": "SEQUENCE_FLOW",
    "sourceRef": { "kind": "step", "name": "JSON to XML Converter" },
    "targetRef": { "kind": "step", "name": "Set Props" }
  }
}
```
...
修改 step 时必须提供完整语义对象或 schema 化 patch；不能只修改低层 `ifl:property`。

```json
{
  "tool": "updateScriptStep",
  "arguments": {
    "iflowId": "iflow_001",
    "processName": "Integration Process",
...
EdgeRepairStrategy =
  LEAVE_DISCONNECTED
  CONNECT_PREDECESSORS_TO_SUCCESSORS
