...
LLM 不直接读取、生成或修改结构化 DSL。系统边界必须是：
...
因此，文档里的 DSL 语法和 JSON 示例是给后端、tool 设计、validator、compiler 和 reviewer 使用的，不是要求 LLM 在 prompt 中读写完整 DSL。
...
## 4. 后端 / Tool 设计用结构化 DSL 示例

以下语法是设计草案，用于说明 DSL 抽象层次。后端可以把同一模型序列化成 JSON，但 LLM 不直接读写这些 DSL 片段；LLM 只调用 tools，由 tool 层把业务语义命令映射为 DSL 变更。
...
Tools、validation、semantic diff 使用 semantic name；compiler projection 使用 `sapName`。如果 SAP 重新生成 MessageFlow ID，只需要更新 projection alias，不应影响业务 DSL。
...
工具可以继续走 JSON RPC 风格，但 LLM 只看到 tool schema。Tool 参数应面向业务语义 action，后端再映射为内部 DSL 变更；不能要求 LLM 提交任意 DSL JSON patch。

每个 state mutation tool 的成功返回必须包含：

- `revision`: 新 DSL revision。
...
Tool 不应把 Java exception、SAP stack trace、完整 BPMN XML 或完整 DSL dump 直接返回给 LLM。低层信息应进入 diagnostics / trace，并用 `errorCode`、`affectedObject`、`suggestedFixes` 摘要给 LLM。
...
- LLM-facing state mutation tool 不允许接受 arbitrary DSL JSON patch。
- Tool result 不允许默认返回完整 DSL；必须返回 compact state summary。
